...
temp = np.load("./two_part_network.npz")
dat1 = temp["dat1"]
dat2 = temp["dat2"]
...
def test_ddn_org_parallel():
    omega1, omega2 = ddn.ddn_parallel(
        dat1, dat2, lambda1=lambda1, lambda2=lambda2, mthd="org", n_process=2
    )
    d1, d2 = get_err(omega1, omega2, omega1_ref, omega2_ref)
    assert d1 < ERR_THRESHOLD
    assert d2 < ERR_THRESHOLD
...
def test_ddn_resi_parallel():
    omega1, omega2 = ddn.ddn_parallel(
        dat1, dat2, lambda1=lambda1, lambda2=lambda2, mthd="resi", n_process=2
    )
    d1, d2 = get_err(omega1, omega2, omega1_ref, omega2_ref)
    assert d1 < ERR_THRESHOLD
    assert d2 < ERR_THRESHOLD
...
def test_ddn_corr_parallel():
    omega1, omega2 = ddn.ddn_parallel(
        dat1, dat2, lambda1=lambda1, lambda2=lambda2, mthd="corr", n_process=2
    )
    d1, d2 = get_err(omega1, omega2, omega1_ref, omega2_ref)
    assert d1 < ERR_THRESHOLD
    assert d2 < ERR_THRESHOLD
